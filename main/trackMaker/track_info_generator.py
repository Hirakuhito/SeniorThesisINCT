...
def gen_mesh_data(points, width, radius, in_out):
    ...
    if in_out == "in":
        ...
        in_offset = normal_unit * ((width / 2) + (radius * 0.8))
        out_offset = normal_unit * ((width / 2) + (radius * 0.5))

        in_right_points = points + offset_vector
        in_left_points = points + in_offset
        out_right_points = points - offset_vector
        out_left_points = points - out_offset
...
def export_obj(mesh_points, filename, in_out):
    ...
    if in_out == 'in':
        ...
        joined_verticies = np.vstack((in_left_points, in_right_points, out_left_points, out_right_points)) #* (N, 8) -> (4N, 2)
